...
static btSoftBody* Ctor_SoftBox(SoftDemo* pdemo,const btVector3& p,const btVector3& s)
{
	...
	btSoftBodyMeshModifier::generateBendingConstraints(softShape, 2);
...
static void	Init_Sticks(SoftDemo* pdemo)
{
	...
	for(int y=0;y<n;++y)
	{
		for(int x=0;x<n;++x)
		{
			...
			for(int i = 0; i < 3; ++i) btSoftBodyMeshModifier::generateBendingConstraints(softShape, 2 + i);
			...
			case	0:
			{
				...
				if(!m_pickConstraint)
				{
					...
					{				
						...
						{
							btSoftBodyFace& face = hitSoftBody->m_softShape->m_faces[m_results.index];
