...
void GpuConvexScene::setupScene()
{
	...
	m_data->m_rigidBodyPipeline->m_aabbs.writeAabbsToGpu();
	m_data->m_rigidBodyPipeline->m_collidables.writeCollidablesToGpu();
	m_data->m_rigidBodyPipeline->m_rigidBodies.writeAllBodiesToGpu();
...
int	GpuConvexScene::createDynamicsObjects2( const float* vertices, int numVertices, const int* indices, int numIndices)
{
...

	{
		...
		if (gUseInstancedCollisionShapes)
			colIndex = m_data->m_rigidBodyPipeline->m_collidables.registerConvexHullShape(utilPtr);
		...
		for (int i=0;i<gGpuArraySizeX;i++)
		{
			...
			for (int j=0;j<gGpuArraySizeY;j++)
			{
				...
				for (int k=0;k<gGpuArraySizeZ;k++)
				{
					...
					int pid = m_data->m_rigidBodyPipeline->m_rigidBodies.registerRigidBody(colIndex, mass, position, orn);
					m_data->m_rigidBodyPipeline->m_aabbs.createProxy(b3MakeVector3(0, 0, 0), b3MakeVector3(0, 0, 0), pid, 0, 0);
...
void GpuConvexScene::createStaticEnvironment()
{
...

	{
		...
		int colIndex = m_data->m_rigidBodyPipeline->m_collidables.registerConvexHullShape(&cube_vertices[0],strideInBytes,numVertices, scaling);
		...
		int pid = m_data->m_rigidBodyPipeline->m_rigidBodies.registerRigidBody(colIndex, 0.f, position, orn);
		m_data->m_rigidBodyPipeline->m_aabbs.createLargeProxy(b3MakeVector3(0, 0, 0), b3MakeVector3(0, 0, 0), pid, 0, 0);
