...
void	GpuRigidBodyDemo::initPhysics()
{
	...
	if (m_clData->m_clContext)
	{
		...
		m_data->m_rigidBodyPipeline = new b3GpuRigidBodyPipeline(m_clData->m_clContext,m_clData->m_clDevice,m_clData->m_clQueue);
...
void GpuRigidBodyDemo::stepSimulation(float deltaTime)
{
	...
	bool animate=true;
	int numObjects = m_data->m_rigidBodyPipeline->m_rigidBodies.m_numRigidBodies;
	//printf("numObjects=%d\n",numObjects);
	...
	{
		{
			...
			int ciErrNum = 0;
			cl_mem bodies = m_data->m_rigidBodyPipeline->m_rigidBodies.m_bodyBufferGPU.getBufferCL();
